...
# Description: Security Baseline Check Script for 3.2.8
#
# #######################################################################################
...
check_ipv4_policy() {
    ...
    for port in "${ports[@]}"; do
        for protocol in "${protocols[@]}"; do
            local rule=$(iptables -L INPUT -v -n | grep -E "$protocol.*dpt:$port")
            if [[ -z "$rule" ]]; then
    ...
    # 检查默认策略
    local default_policy=$(iptables -L INPUT -n | grep "Chain INPUT" | awk '{print $4}' | tr -d '()' )
    if [[ "$default_policy" != "DROP" ]]; then
        echo "IPv4 的 INPUT 默认策略不是 DROP。当前策略为 $default_policy。"
        failure=1
    else
        echo "IPv4 的 INPUT 默认策略已设置为 DROP。"
    fi
...
check_ipv6_policy() {
    ...
    for port in "${ports[@]}"; do
        for protocol in "${protocols[@]}"; do
            local rule=$(ip6tables -L INPUT -v -n | grep -E "$protocol.*dpt:$port")
            if [[ -z "$rule" ]]; then
    ...
    # 检查默认策略
    local default_policy=$(ip6tables -L INPUT -n | grep "Chain INPUT" | awk '{print $4}' | tr -d '()')
    if [[ "$default_policy" != "DROP" ]]; then
        echo "IPv6 的 INPUT 默认策略不是 DROP。当前策略为 $default_policy。"
        failure=1
    else
        echo "IPv6 的 INPUT 默认策略已设置为 DROP。"
    fi
